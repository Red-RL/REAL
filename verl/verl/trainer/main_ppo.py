...
def main(config):
    print('lingze-debug ray.is_initialized:',ray.is_initialized())
    if not ray.is_initialized():
        # this is for local ray cluster
        import os
        os.environ["TOKENIZERS_PARALLELISM"] = "true"
        os.environ["NCCL_lingze"] = "WARN"

        # lingze-debug: for lingze-debug
        import subprocess
        my_ip = subprocess.check_output(["hostname", "-i"]).decode().strip()
        print(f"lingze-debug-----{my_ip}:8265")
        ...
        # ray.init(runtime_env={'env_vars': {'TOKENIZERS_PARALLELISM': 'true', 'NCCL_lingze': 'WARN'}})
